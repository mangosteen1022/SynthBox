...
class MSALClient:
    ...
    def _init_cache_path(self, token_uuid: Optional[str]):
        ...
        # 向上两级（core/auth/ -> core/ -> outlook/ -> tokens/）
        token_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(base_dir))), "tokens")
    ...
    def list_messages_since(self, days_ago: int = 7, **kwargs) -> Dict[str, Any]:
        """获取指定天数内的邮件"""
        filter_str = f"receivedDateTime gt {self.dt_helper.days_ago(days_ago)}"
    ...
    def get_messages_delta(self, delta_link: Optional[str] = None) -> Dict[str, Any]:
        """
        获取邮件变更（Delta查询）
        ...
        if delta_link:
            ...
            return self._graph_request("GET", "me/messages/delta", params=params, timeout=30)
