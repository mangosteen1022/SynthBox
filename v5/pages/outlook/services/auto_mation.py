...
from v5.core.CheckProxy import CheckProxyByProxyGenerate
from v5.core.EmailClientByApi import EmailClientByApi
from v5.core.utils import ProxyPlatform, sess_edition, capture_error, log
from v5.pages.new_outlook.config.domain import domain
...
class Worker(QRunnable):
    ...
    # @staticmethod
    # def get_device_id():
    #     with open("js/device_id.js", "r", encoding="utf-8") as f:
    #         js_code = f.read()
    #     js = execjs.compile(js_code)
    #     return js.call("device_id")
    ...
    def accrue_procedure(self, html):
        try:
            ...
            log.debug(f"procedure_type1 {str(e)}")
            try:
                self.procedure_type2(html)
            except Exception as e:
                log.debug(f"procedure_type2  {str(e)}")
                try:
